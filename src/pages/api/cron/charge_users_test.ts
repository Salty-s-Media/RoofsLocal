...
// localhost:3000/api/cron/charge_users_test?dryRun=true
...
export default async function handler(
  ...
  if (dryRun) console.log('=== DRY RUN MODE — no charges, no GHL, no HubSpot writes ===');
  ...
  try {
    ...
    if (dryRun) {
      // In dry-run, skip everything that mutates state.
      // Just run matching to confirm Prisma connectivity, then report.
      const { map: contractorOpenLeadsMap, unmatched: unmatchedOpen } =
        await matchLeadsReadOnly(openLeads);
      const { assigned: contractorUnpaidLeadsMap, unassigned: unsyncedLeads } =
        await resolveLeadsByCompany(unpaidLeads);
      const { map: contractorUnsyncedLeadsMap, unmatched: unmatchedUnsynced } =
        await matchLeadsReadOnly(unsyncedLeads);

      const mergedLeadsMap = mergeLeadsMaps(
        mergeLeadsMaps(contractorOpenLeadsMap, contractorUnsyncedLeadsMap),
        contractorUnpaidLeadsMap
      );

      // Build per-contractor billing summary with lead details
      const summary: Record<string, any> = {};
      for (const contractorId in mergedLeadsMap) {
        const contractor = await prisma.contractor.findUnique({
          where: { id: contractorId },
        });
        const leads = mergedLeadsMap[contractorId];
        const cost = leads.length * (contractor?.pricePerLead ?? 0);
        summary[contractor?.email ?? contractorId] = {
          leads: leads.length,
          wouldCharge: `$${cost / 100}`,
          pricePerLead: (contractor?.pricePerLead ?? 0) / 100,
          company: contractor?.company ?? 'Unknown',
          leadDetails: leads.map((l: any) => ({
            id: l.id,
            name: `${l.firstname ?? ''} ${l.lastname ?? ''}`.trim() || 'Unknown',
            zip: l.zip?.substring(0, 5) ?? 'N/A',
            createdate: l.createdate ?? null,
            hubspotUrl: HUBSPOT_PORTAL_ID
              ? `https://app.hubspot.com/contacts/${HUBSPOT_PORTAL_ID}/contact/${l.id}`
              : null,
            status: l.company && l.company !== 'Unknown Company' && l.company !== 'Roofs Local'
              ? 'in_progress'
              : 'open',
          })),
        };
        console.log(
          `[DRY RUN] Would charge ${contractor?.email} $${cost / 100} for ${leads.length} leads`
        );
      }

      // Combine all unmatched leads with a reason
      const allUnmatched = [
        ...unmatchedOpen.map((l) => ({ ...l, source: 'open' as const })),
        ...unmatchedUnsynced.map((l) => ({ ...l, source: 'in_progress' as const })),
      ];

      const unmatchedDetails = allUnmatched.map((l) => ({
        id: l.id,
        name: `${l.firstname ?? ''} ${l.lastname ?? ''}`.trim() || 'Unknown',
        zip: l.zip?.substring(0, 5) ?? null,
        createdate: l.createdate ?? null,
        hubspotUrl: HUBSPOT_PORTAL_ID
          ? `https://app.hubspot.com/contacts/${HUBSPOT_PORTAL_ID}/contact/${l.id}`
          : null,
        reason: !l.zip
          ? 'no_zip'
          : 'no_contractor_coverage',
        source: l.source,
      }));

      return res.status(200).json({
        message: 'Dry run completed — no mutations performed',
        openLeads: openLeads.length,
        unpaidLeads: unpaidLeads.length,
        unsyncedLeads: unsyncedLeads.length,
        billing: summary,
        unmatchedLeads: unmatchedDetails,
      });
    }
...
/**
 * Read-only version of matchLeads for dry-run mode.
 * Finds the best contractor per lead WITHOUT incrementing zip counts.
 * Exercises the same Prisma queries so we can verify connectivity.
 *
 * Returns both matched leads (map) and unmatched leads (array) so the
 * caller can surface "no contractor coverage" info in the preview.
 */
async function matchLeadsReadOnly(leads: any[]): Promise<{
  map: { [key: string]: any[] };
  unmatched: any[];
}> {
  const contractorLeadsMap: { [key: string]: any[] } = {};
  ...
  for (const lead of leads) {
    try {
      const zip = lead.zip?.substring(0, 5);
      if (!zip) {
        console.warn(`[DRY RUN] Lead ${lead.id} has no zip code, skipping`);
        unmatched.push(lead);
        continue;
      }
      ...
      if (contractors.length === 0) {
        console.log(`[DRY RUN] No contractors for zip ${zip}, skipping lead ${lead.id}`);
        unmatched.push(lead);
        continue;
      }
      ...
      const existingCounts = await prisma.contractorZipCount.findMany({
        where: {
          zipCode: zip,
          contractorId: { in: contractorIds },
        },
      ...
      console.log(
        `[DRY RUN] Lead ${lead.id} (zip ${zip}) → ${bestContractor.company} (count: ${bestCount}, tied: ${candidates.length})`
      );

      if (!contractorLeadsMap[bestContractor.id]) {
        contractorLeadsMap[bestContractor.id] = [];
      }
      contractorLeadsMap[bestContractor.id].push(lead);
    } catch (error) {
      console.error(`[DRY RUN] Failed to match lead ${lead.id}:`, error);
      unmatched.push(lead);
      continue;
    }
  }

  return { map: contractorLeadsMap, unmatched };
}
